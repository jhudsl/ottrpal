...
However, if you wish to skip this step, you can set `render = FALSE` when running the `ottr::bookdown_to_embed_leanpub()` function.
...
```{r}
ottr::bookdown_to_embed_leanpub(make_book_txt = TRUE)
```
...
```{r}
ottr::bookdown_to_embed_leanpub(quiz_dir = "some_directory")
```
...
ottr::bookdown_to_embed_leanpub(footer_text = survey_link)
```
