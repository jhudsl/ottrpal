...
#'
#' @param quiz_specs The output from [ottrsidekick::parse_quiz].
...
#'
#' This is ran automatically by [ottrsidekick::check_all_questions] for all questions.
#' It checks that the attributes specified are accepted ones by Leanpub.
#'
#' @param question_df a data.frame obtained from [ottrsidekick::parse_quiz_df] and dplyr::group_split(question).
...
#'
#' Takes output from [ottrsidekick::parse_quiz] and runs checks on each question in a quiz by calling [ottrsidekick::check_question] for each question.
#' First splits questions into their own data frame. Returns a list of messages/warnings about each question's set up.
#'
#' @param quiz_specs quiz_specs which is output from [ottrsidekick::parse_quiz].
#' @param quiz_name The name of the quiz being checked.
#' @param verbose Whether progress messages should be given.
#'
#' @return A list of the output from [ottrsidekick::check_question] with messages/warnings regarding each question and each check.
#'
...
#' Is called by [ottrsidekick::check_all_questions] and run for each question.
#'
...
#'
#' @param quiz_dir A path to a directory full of quizzes that should all be checked with [ottrsidekick::check_all_quizzes].
#' @param verbose print diagnostic messages
